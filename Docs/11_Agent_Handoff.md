...
23. Developer trust strategy lives in `Docs/35_Developer_Trust_Strategy.md`; developer-tools market positioning lives in `Docs/38_Developer_Tools_Market_Positioning.md`; Truth Centre stress gates live in `Docs/36_Truth_Centre_Stress_Plan.md`.
...
26. Truth Debt gates live in `TruthDebtEngine`, `TruthDebtModels.swift`, `TruthDebtGateTests`, and `Docs/40_Truth_Debt_Gates.md`. Keep them local, selected-project-scoped, read-only, deterministic, and distinct from the Truth Centre percentage. Critical/High in-scope debt blocks release-ready claims; lower-priority debt remains a caveat unless tied to a blocking risk or condition. UI surfaces must remain thin read-only presentations of the deterministic report; do not imply persisted debt records, automatic release approval, background scanning, command execution, or repository mutation unless that runtime code exists and is validated.
27. Truth Audit Packet direction lives in `Docs/41_Truth_Audit_Packets.md`. Treat it as a documentation-only handoff contract until code exists. Future packets should tie Reality score, Confidence, contribution provenance, Truth Debt gates, and Release Readiness wording together, but release-ready claims remain blocked or caveated until source-linked evidence supports them. Do not claim Metal/background-rendering work, release PRs, CI/signing/notarisation results, or upstream changes are merged unless upstream `main` contains them and validation is recorded.
28. Handoff trust boundaries live in `Docs/43_Handoff_Trust_Boundaries.md`. Use them when writing agent handoffs, PR summaries, release handoffs, or market claims. A handoff may claim only scoped local state, commands actually run, source-linked records, and explicit caveats. It must not claim release-ready status, security completeness, CI/signing/notarisation results, upstream merge state, or another agent's work unless the evidence is carried. PR slices should state the branch/base, changed files, validation run, blockers, unknowns, and exact review path.
