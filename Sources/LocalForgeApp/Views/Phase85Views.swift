...
struct BackupCentreView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text("Local-only backups of the workspace database. Last \(backups.count) kept on disk.")
                .foregroundStyle(.secondary)
            if let error = error {
                ...
                VStack(spacing: 6) {
                    ForEach(backups) { b in
                        HStack(spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                Text(b.filename).font(.system(size: 13, weight: .medium)).lineLimit(1)
                                Text("\(b.createdAt.formatted(date: .abbreviated, time: .shortened)) · \(b.sizeDisplay)")
                                    .font(.caption).foregroundStyle(.secondary)
                            }
                            Spacer()
                            Button("Restore") { showRestoreConfirm = b }
                                .controlSize(.small)
    ...
    private func refresh() {
        do { backups = try store.backupEngine.listBackups() } catch { self.error = error.localizedDescription }
    }
    ...
    private func createBackup() {
        ...
        do {
            _ = try store.backupEngine.createBackup(from: sqliteURL, note: note)
            note = ""
            error = nil
            refresh()
        } catch {
    ...
    private func restore(_ backup: BackupRecord) {
        guard let sqliteURL = currentWorkspaceURL() else { return }
        do {
    ...
    private func delete(_ backup: BackupRecord) {
        do { try store.backupEngine.delete(backup: backup); refresh() }
